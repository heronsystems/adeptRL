...
def make_atari_env(env_id, env_conf, args, seed, frame_stack=False):
    def _f():
        env = atari_env(env_id, env_conf, args, frame_stack)
        env.seed(seed)
        return env
    return _f


def atari_env(env_id, env_conf, args, frame_stack=False):
    env = gym.make(env_id)
