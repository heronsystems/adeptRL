...
from adept.actor import DQNRolloutActor
from adept.exp import Rollout
from adept.learner import DQNRolloutLearner
from .base.agent_module import AgentModule


class DQNRollout(AgentModule):
    args = {
        **Rollout.args,
        **DQNRolloutActor.args,
        **DQNRolloutLearner.args
    }
    ...
    def __init__(
        ...
        super(DQNRollout, self).__init__(
            reward_normalizer,
            action_space
        )
        self._exp_cache = Rollout(spec_builder, rollout_len)
        self._actor = DQNRolloutActor(action_space, nb_env)
        self._learner = DQNRolloutLearner(
            reward_normalizer,
            discount,
            return_scale,
            double_dqn
    ...
    def _exp_spec(cls, exp_len, batch_sz, obs_space, act_space, internal_space):
        return DQNRolloutActor._exp_spec(
            exp_len, batch_sz, obs_space, act_space, internal_space
        )

    @staticmethod
    def output_space(action_space):
        return DQNRolloutActor.output_space(action_space)
