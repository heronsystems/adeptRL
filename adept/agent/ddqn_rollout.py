...
from adept.actor import DDQNRolloutActor
from adept.exp import Rollout
from adept.learner import DDQNRolloutLearner
from .base.agent_module import AgentModule


# TODO: this is just a copy of DQNRollout, should be a way to just combine these
class DDQNRollout(AgentModule):
    args = {
        **Rollout.args,
        **DDQNRolloutActor.args,
        **DDQNRolloutLearner.args
    }
    ...
    def __init__(
        ...
        super(DDQNRollout, self).__init__(
            reward_normalizer,
            action_space
        )
        self._exp_cache = Rollout(spec_builder, rollout_len)
        self._actor = DDQNRolloutActor(action_space, nb_env)
        self._learner = DDQNRolloutLearner(
            reward_normalizer,
            discount,
            return_scale,
            double_dqn
    ...
    def _exp_spec(cls, exp_len, batch_sz, obs_space, act_space, internal_space):
        return DDQNRolloutActor._exp_spec(
            exp_len, batch_sz, obs_space, act_space, internal_space
        )

    @staticmethod
    def output_space(action_space):
        return DDQNRolloutActor.output_space(action_space)
