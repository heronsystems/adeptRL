...
class RolloutWorkerManager:
    def __init__(self, workers, min_batch, queue_max_size=0, async_queue=False):
        ...
        if not async_queue:
            self.queue_max_size = min_batch
            self.sync_batch_lock = threading.Lock()
        else:
    ...
    def _background_queing_thread(self):
        while not self._should_stop:
            with self.sync_batch_lock:
                ...
                if len(ready_ids) == 0:
                    # if synchronous pause to let the batch run through instead of busy looping
                    if not self.async_queue:
                        self.sync_batch_lock.release()
                        # no need to sleep here since get() should be waiting to acquire
                        # self.sync_batch_lock.acquire()
                    print('wasted')
    ...
    def get(self):
        batch = [self.rollout_queue.get(True) for _ in range(self.min_batch)]
        self.sync_batch_lock.acquire()
        # TODO: agg batch
        return batch
    ...
    def resume(self):
        """
            ...
            Should be called only after workers have received weights and ack'd
        """
        assert not self.async_queue
        self._restart_idle_workers()
        self.sync_batch_lock.release()
