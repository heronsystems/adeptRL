...
from torch.nn import ConvTranspose2d, BatchNorm1d, BatchNorm2d, init, functional as F
...
class Embedder(NetworkModule):
    ...
    def __init__(self, args, obs_space, output_space):
        ...
        if self._vae:
            # reparameterize
            self.lstm = LSTM(conv_stack_flat_shape, 'lstm', True, 1024)
            embed_shape = list(self.lstm.output_shape())
            # don't include mu & sigma
            embed_shape[0] = 512
            embed_shape = tuple(embed_shape)
        else:
            self.lstm = LSTM(conv_stack_flat_shape, 'lstm', True, 512)
        ...
        if self._autoencoder:
            self.ae_upsample = DeconvLSTM(embed_shape[0])

        # predict reward negative, zero, positive given encoded_obs and action
        if self._reward_pred:
            self.reward_pred = nn.Sequential(
                nn.Linear(embed_flat_shape+self._nb_action, 128, bias=False),
                nn.BatchNorm1d(128),
                nn.ReLU(),
                nn.Linear(128, 3),
                nn.Softmax(dim=1)
        ...
        if self._next_embed_pred:
            if self._additive_embed:
                self.embed_pred = nn.Sequential(
                    nn.Linear(self._nb_action, 1024, bias=False),
                    nn.BatchNorm1d(1024),
                    nn.ReLU(),
                    nn.Linear(1024, 512),
                    nn.Tanh()
                ...
                self.embed_pred = nn.Sequential(
                    nn.Linear(embed_flat_shape+self._nb_action, 1024, bias=False),
                    nn.BatchNorm1d(1024),
                    nn.ReLU(),
                    nn.Linear(1024, 512),
                    nn.Tanh()
        ...
        if self._inv_model:
            self.inv_pred = nn.Sequential(
                nn.Linear(embed_flat_shape * 2, 1024, bias=False),
                nn.BatchNorm1d(1024),
                nn.ReLU(),
                nn.Linear(1024, 512, bias=False),
                nn.BatchNorm1d(512),
                nn.ReLU(),
                nn.Linear(512, self._nb_action),
                nn.Softmax(1)
            )
    ...
    def forward(self, observation, internals, ret_imag=False):
        ...
        pol_outs = {k: self.pol_outputs[k](encoded_obs) for k in self.pol_outputs.keys()}
...
class DeconvLSTM(nn.Module):
    def __init__(self, in_channel, batch_norm=False):
        super().__init__()
        self._out_shape = None
        self.lin1 = nn.Linear(in_channel, 16*3*3)
        self.conv1 = ConvTranspose2d(16, 32*4, 7, bias=False)
        ...
        self.ln1 = BatchNorm1d(16*3*3)

        relu_gain = init.calculate_gain('relu')
        self.lin1.weight.data.mul_(relu_gain)
        self.conv1.weight.data.mul_(relu_gain)
        self.conv2.weight.data.mul_(relu_gain)
        self.conv3.weight.data.mul_(relu_gain)

    def forward(self, xs):
        xs = F.leaky_relu(self.ln1(self.lin1(xs)))
        xs = xs.view(-1, 16, 3, 3)
